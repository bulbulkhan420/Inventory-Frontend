...
export const CatagoryUpdate = ({
    ...
    let updatecatagory:SubmitHandler<InputCatagory>=async(data)=>{
        let result=await catagoryUpdateService(`${catagoryUpdate}`,catagoryId ,data?.catagory);
    ...
    return (
        <div>
           ...
           }} className="w-screen h-screen  flex justify-center items-center bg-gray-50 bg-opacity-50 fixed z-20 top-0 left-0">
                
                <form  onSubmit={handleSubmit(updatecatagory)} className=" bg-gray-200 shadow-md w-[400px] h-auto p-20 rounded-md flex justify-center items-center gap-y-4 flex-col" >
              
                <input
                        className="w-full h-[40px] rounded-md text-center border-2"
                        type="text"
                        
                        {...register('catagory',{required:true,minLength:3})}
                        defaultValue={catagoryName ? catagoryName : ""}
                    />
                    <button type="submit" className="w-full h-[40px] rounded-md bg-blue-600 text-white flex justify-center items-center">
                        Update
                    </button>
                    <div
                        onClick={() => {
